...
sites = list(pd.read_csv("WebsiteLists/AtheistSites.csv")["url_domain"])
sites = [
...
browser_params = [
    BrowserParams(
        ...
        profile_archive_dir=Path("./ProfileLists/ProfileAS"),
    )
...
with TaskManager(
    manager_params,
    browser_params,
    SQLiteStorageProvider(Path("./datadir/crawl-dataAS.sqlite")),
    None,
) as manager:
    # Visits the sites
