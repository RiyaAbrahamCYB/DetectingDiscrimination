...
sites = list(pd.read_csv("WebsiteLists/BuddhistSites.csv")["url_domain"])
sites = [
...
browser_params = [
    BrowserParams(
        ...
        profile_archive_dir=Path("./ProfileLists/ProfileBS"),
    )
...
with TaskManager(
    manager_params,
    browser_params,
    SQLiteStorageProvider(Path("./datadir/crawl-dataBS.sqlite")),
    None,
) as manager:
    # Visits the sites
