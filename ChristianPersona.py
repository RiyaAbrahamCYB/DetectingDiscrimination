...
sites = list(pd.read_csv("WebsiteLists/ChristianSites.csv")["url_domain"])
sites = [
...
browser_params = [
    BrowserParams(
        ...
        profile_archive_dir=Path("./ProfileLists/ProfileCS"),
    )
...
with TaskManager(
    manager_params,
    browser_params,
    SQLiteStorageProvider(Path("./datadir/crawl-dataCS.sqlite")),
    None,
) as manager:
    # Visits the sites
