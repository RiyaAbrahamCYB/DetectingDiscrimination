...
sites = list(pd.read_csv("WebsiteLists/Female.csv")["url_domain"])
sites = [
...
browser_params = [
    BrowserParams(
        ...
        profile_archive_dir=Path("./ProfileLists/ProfileFemale"),
    )
...
with TaskManager(
    manager_params,
    browser_params,
    SQLiteStorageProvider(Path("./datadir/crawl-dataFemale.sqlite")),
    None,
) as manager:
    # Visits the sites
