...
sites = list(pd.read_csv("WebsiteLists/FemaleNew.csv")["url_domain"])
sites = [
...
browser_params = [
    BrowserParams(
        ...
        profile_archive_dir=Path("./ProfileLists/ProfileFN"),
    )
...
with TaskManager(
    manager_params,
    browser_params,
    SQLiteStorageProvider(Path("./datadir/crawl-dataFN.sqlite")),
    None,
) as manager:
    # Visits the sites
