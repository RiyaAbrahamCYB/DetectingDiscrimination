...
sites = list(pd.read_csv("WebsiteLists/IslamicSites.csv")["url_domain"])
sites = [
...
browser_params = [
    BrowserParams(
        ...
        profile_archive_dir=Path("./ProfileLists/ProfileIS"),
    )
...
with TaskManager(
    manager_params,
    browser_params,
    SQLiteStorageProvider(Path("./datadir/crawl-dataIS.sqlite")),
    None,
) as manager:
    # Visits the sites
