...
sites = list(pd.read_csv("WebsiteLists/Male.csv")["url_domain"])
sites = [
...
browser_params = [
    BrowserParams(
        ...
        profile_archive_dir=Path("./ProfileLists/ProfileMale"),
    )
...
with TaskManager(
    manager_params,
    browser_params,
    SQLiteStorageProvider(Path("./datadir/crawl-dataMale.sqlite")),
    None,
) as manager:
    # Visits the sites
