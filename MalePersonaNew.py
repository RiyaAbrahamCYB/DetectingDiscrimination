...
sites = list(pd.read_csv("WebsiteLists/MaleNew.csv")["url_domain"])
sites = [
...
browser_params = [
    BrowserParams(
        ...
        profile_archive_dir=Path("./ProfileLists/ProfileMN"),
    )
...
with TaskManager(
    manager_params,
    browser_params,
    SQLiteStorageProvider(Path("./datadir/crawl-dataMN.sqlite")),
    None,
) as manager:
    # Visits the sites
