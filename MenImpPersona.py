...
sites = list(pd.read_csv("WebsiteLists/MentalImpairment.csv")["url_domain"])
sites = [
...
browser_params = [
    BrowserParams(
        ...
        profile_archive_dir=Path("./ProfileLists/ProfileMI"),
    )
...
with TaskManager(
    manager_params,
    browser_params,
    SQLiteStorageProvider(Path("./datadir/crawl-dataMI.sqlite")),
    None,
) as manager:
    # Visits the sites
