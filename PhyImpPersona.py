...
sites = list(pd.read_csv("WebsiteLists/PhyImpairment.csv")["url_domain"])
sites = [
...
browser_params = [
    BrowserParams(
        ...
        profile_archive_dir=Path("./ProfileLists/ProfilePI"),
    )
...
with TaskManager(
    manager_params,
    browser_params,
    SQLiteStorageProvider(Path("./datadir/crawl-dataPI.sqlite")),
    None,
) as manager:
    # Visits the sites
