...
sites = list(pd.read_csv("WebsiteLists/Senior.csv")["url_domain"])
sites = [
...
browser_params = [
    BrowserParams(
        ...
        profile_archive_dir=Path("./ProfileLists/ProfileSenior"),
    )
...
with TaskManager(
    manager_params,
    browser_params,
    SQLiteStorageProvider(Path("./datadir/crawl-dataS.sqlite")),
    None,
) as manager:
    # Visits the sites
