...
sites = list(pd.read_csv("WebsiteLists/Young.csv")["url_domain"])
sites = [
...
browser_params = [
    BrowserParams(
        ...
        profile_archive_dir=Path("./ProfileLists/ProfileYoung"),
    )
...
with TaskManager(
    manager_params,
    browser_params,
    SQLiteStorageProvider(Path("./datadir/crawl-dataY.sqlite")),
    None,
) as manager:
    # Visits the sites
